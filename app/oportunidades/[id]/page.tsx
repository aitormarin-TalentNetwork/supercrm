...
// `taxRate` se guarda como fracción (0-1), no como entero de 0 a 100 — un
// IVA de 21,5% se persiste como 0.215. Mostrar/precargar ese valor exige
// alguna precisión fija (para no arrastrar ruido de coma flotante como
// 21.499999999999996), pero un redondeo solo-en-UI es con pérdida si el
// servidor acepta más decimales de los que aquí se muestran: un IVA
// guardado con más precisión se corrompía silenciosamente en el primer
// resave sin tocar el campo (ronda de auditoría 3), y seguía pudiendo
// perder precisión con valores más extremos aunque el punto de corte
// subiera (ronda de auditoría 4, mismo problema en un dominio más
// amplio). `roundTaxRate` (lib/quoteMath.ts) es el mismo contrato de
// precisión que aplica convex/quotes.ts ANTES de persistir — ningún
// taxRate guardado puede tener ya más decimales de los que se formatean
// aquí, así que este redondeo deja de tener pérdida posible.
