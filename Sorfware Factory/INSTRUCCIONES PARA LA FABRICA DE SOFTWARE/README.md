...
| `SendMessage` con `notify_when_idle: true` como aviso de "terminal libre" | **NO VERIFICADO en esta fábrica** | Es el mecanismo nativo para sesiones de Claude Code —a diferencia del patrón de marker, que existe porque el auditor es un proceso externo— y es la pieza de la decisión 26.1. Está documentado, pero **nadie lo ha ejercido aquí todavía**: la Directora tiene que usarlo una vez de verdad y confirmar que el aviso llega. Hoy ya hemos aprendido tres veces qué pasa con lo plausible. ⚠️ Y lo que **sí** está claro por diseño: **no dispara con una sesión atascada en un prompt**, porque esa sesión no está ociosa — por eso la 26.2 añade un temporizador aparte. |
