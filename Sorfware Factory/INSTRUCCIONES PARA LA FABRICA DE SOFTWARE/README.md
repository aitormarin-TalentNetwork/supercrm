...
|---|---|---|
| 🔴 **El ORDEN de los casos de un test, cuando los que esperan verde van primero** | **Decide si el test puede detectar que el ARNÉS está roto.** Si los casos que esperan PERMITE se ejecutan antes, **un arnés que no ejecuta nada da verde y parece sano**. Caso del 2026-09-10, del CEO escribiendo el test del hook de secretos: dio **3/7**, y los cuatro fallos eran casos PERMITE porque el arnés perdía el hook tras el primer caso. **Se vio SOLO porque los cinco casos que BLOQUEAN iban primero** — con los dos PERMITE al principio habría leído *2/2 verdes* y se habría quedado tranquilo **con un hook inexistente**. 📌 **Y el agravante, que es lo caro:** su primera explicación —*"`git clean` borra `.githooks` por estar sin trackear"*— era plausible, **la escribió DENTRO del script como comentario**, y era falsa: con `-e .githooks` seguía dando 3/4. **Una explicación plausible escrita dentro del instrumento es un desactivador de comprobaciones en el peor sitio posible**, porque el siguiente que lea el script la dará por diagnóstico hecho y no volverá a medir | **Control positivo del ARNÉS en cada caso**, no al principio: antes de cada uno, comprobar que el instrumento sigue vivo. Así deja de depender del orden. Es la enmienda 9 —*si no habría podido salir mal, no es una comprobación*— **con un sujeto nuevo: no la prueba, el andamio que la corre.** Y al diagnosticar: **medir, no razonar** — el arreglo real era tener el hook TRACKEADO para que `reset --hard` lo devuelva |
| **Auditar siguiendo las referencias del código (del consumidor a la función)** | **La RUTA decide qué explicaciones te encuentras, y las explicaciones NO son neutrales: están colocadas justo donde alguien previó que mirarías.** Caso del 2026-09-10, del QA: `convex/users.ts` tiene una query pública sin guarda con un comentario de *"Anti-enumeración"* encima, y su consumidor razona bien sobre anti-enumeración y concluye que eso evita filtrar qué emails están dados de alta — **correcto sobre el diálogo, falso sobre el sistema**. Él llegó **enumerando exportaciones**; si hubiera llegado leyendo al consumidor, ese comentario lo habría absuelto. **Lo que le salvó no fue mirar mejor: fue entrar por un camino donde ese texto no estaba** | **Enumerar artefactos**, no seguir enlaces. Corolario: *enumerar produce evidencia sobre el CONJUNTO; seguir referencias produce evidencia sobre el CAMINO* — y el camino es el que el autor anticipó |
| 🔴 **Y la otra mitad: enumerar protege de las explicaciones, NO protege del FILTRO** | **Un filtro escrito de memoria está tan sesgado como un comentario colocado a propósito, con el agravante de que el sesgo es TUYO y no lo vas a reconocer.** Caso del mismo día, del Factory Architect reproduciendo el hallazgo anterior: enumerando le salieron **12 de 28 queries "sin guarda"**, y **once eran falsos** — `convex/dashboard.ts` dice literalmente que todas sus queries usan `requireStoreAccess`, y **esa guarda no estaba en su patrón porque escribió la lista de memoria en vez de leerla del código**. Rehecho con el patrón construido desde los `export` reales de `convex/model/access.ts`: **1 de 28**, la del QA y ninguna más. ⚠️ Con once falsos de doce, una enumeración **se abandona al segundo intento** (46) y se vuelve a auditar siguiendo referencias, que es justo lo que falla | **El filtro se construye desde una fuente que se pueda AGOTAR, nunca de memoria. Y se dice cuántos se miraron.** Es la decisión 64 con un sujeto nuevo: **el patrón de un barrido** |
| **Un texto que declara una protección que no existe** | **No es documentación desactualizada: es un DESACTIVADOR DE COMPROBACIONES.** Le ahorra a quien lo lee exactamente la comprobación que habría destapado el hueco — un *"eso no existe"* silencia mejor que el silencio. Caso: `ceo.md` afirmando que `.githooks` no existía cuando llevaba un día existiendo, inerte | **Se corrige en su propio plazo, DESACOPLADO del arreglo técnico**: su daño existe **mientras el bug sigue vivo**, no después, así que meterlo en la misma ficha que el arreglo lo ata a una fecha que no le corresponde |
| **Un control positivo que exige encontrar hoy lo que el trabajo ya corrigió** | **Caduca cuando el trabajo que vigila avanza, y caduca HACIA EL VERDE.** Caso del 2026-09-10, de T1 en AIT-99: el plan §7.12 exigía que cuatro frases documentales aparecieran al menos una vez *"porque un 0 significaría que busco mal, no que esté hecho"*. Hoy dan **0 porque ya están corregidas** — el control se diseñó contra un estado del repo **que el propio trabajo superó**, y el `0` pasa a ser ambiguo **en la dirección contraria a la que el plan previó** | **buscar ancho y comprobar cuál de las dos cosas significa el cero** (ella lo hizo y encontró las cuatro frases ya corregidas). Y al escribir un control positivo, **elegir un sujeto que el propio trabajo no vaya a mover** — un artefacto muerto o congelado, nunca algo que la tarea está a punto de cambiar |
...
|---|---|---|
| **Hook de secretos `.githooks/pre-commit`** (decisión 33) | **EXISTE y DISCRIMINA 7/7 — pero está INERTE, no armado** | **Existe** desde `04f5db1` (2026-09-09), trackeado, 2090 bytes. **Inerte**, medido por tres vías independientes el 2026-09-10: `git config --get core.hooksPath` vacío (CEO y Factory Architect) **y** `.git/hooks/pre-commit` inexistente (PM) — sin esta segunda, *"no está en `core.hooksPath`"* no bastaría para decir inerte. **Hoy no corre por ninguna de las dos rutas.** · **Discrimina en las DOS direcciones (60.1), 7/7 con exit 0**, en repo desechable bajo `$TMPDIR`, nunca sobre el real (18.1): **BLOQUEA** duplicado de macOS con su original al lado · `e2e/.auth/*` · cuarentena dentro del repo · `-----BEGIN RSA PRIVATE KEY-----` · JWT `eyJ...`; **PERMITE** commit limpio · **`Fase 2.md` SIN `Fase.md` al lado**. Ese último es el que lo convierte en prueba: se parece al peligro y no lo es — los cinco bloqueos solos habrían salido igual de verdes sin probar nada (19). · **Reejecutable:** `bash .githooks/probar-pre-commit.sh` (52: *un gate que solo se ejecuta una vez es documentación*). ⚠️ **Su v1 tenía un defecto propio que falla hacia el verde** — ver su fila en §2sexies. · 🔴 **CASO POSITIVO CON FECHA de por qué la activación es de HOY y no "cuando toque":** el Integrador publicó `105cb71..dcb1a77` el 2026-09-10 y anotó literal *"la única comprobación que hubo fue la mía, a mano"*. Cero patrones en el diff, así que no pasó nada — **pero mañana esa publicación se lee limpia en el registro y nadie sabría que el hook no participó.** No es un razonamiento: es una publicación que ya ocurrió. · **Y la 46 está satisfecha con un margen que no se repite:** al medirlo había cero desarrolladores, índice vacío, árbol limpio y cero ficheros trackeados que el hook bloquearía. |
